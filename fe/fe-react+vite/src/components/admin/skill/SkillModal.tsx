...
const SkillModal: React.FC<SkillModalProps> = ({ isOpen, onClose, dataInit, refetch }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<ISkill> = { name };
    ...
    try {
      if (dataInit?._id) {
        ...
        if (res) {
          toast.success(res.message || 'Thêm mới kỹ năng thành công!');
          refetch();
          onClose();
        } else {
          toast.error(res?.message || 'Có lỗi xảy ra khi thêm mới kỹ năng.');
        }
      }
    } catch (error: any) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label htmlFor="skill-name" className="block text-sm font-medium text-gray-700 mb-1">Tên Kỹ năng</label>
            <input type="text" id="skill-name" required value={name} onChange={(e) => setName(e.target.value)} disabled={isLoading} className={inputFieldClass} />
          </div>
